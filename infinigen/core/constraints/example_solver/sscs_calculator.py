...
class SSCSCalculator:
    def __init__(self, norm_params=None):
        self.norm = norm_params or {
            "C_max": 25, "I_max": 500, "S_max": 8000, "F_max": 800, "T_max": 5, "M_max": 50, "P_max": 40000
        }
    ...
    def compute(self, scene):
        ...
        S_LC = 0.4 * E_spatial + 0.3 * (Surface_area / self.norm["S_max"]) + 0.3 * (Floor_area / self.norm["F_max"])
        ...
        SSCS = 0.3 * S_OD + 0.3 * S_LC + 0.1 * S_FP + 0.3 * S_VD
        return SSCS 